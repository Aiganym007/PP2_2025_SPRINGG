...
class Coin(pygame.sprite.Sprite):
    ...
    def move(self):
        pass  # Монета не двигается, она остается в своем месте
...
while True:    
    ...
    # Двигаем все объекты (игрока, врагов, монет)
    ...
    # Если игрок столкнулся с монетой
    if pygame.sprite.spritecollideany(P1, coins):
        pygame.mixer.Sound(r'C:\Users\Gulistan\Desktop\PP2_2025_SPRING\lecture0\racer\catch.mp3').play()
        SCORE += 1
        C1.change()

    # Если не столкнулся с монетой, уменьшаем счет на 30
    else:
        SCORE -= 30

    # Если счет равен 0 или меньше, завершить игру
    if SCORE <= 0:
        pygame.mixer.Sound(r'C:\Users\Gulistan\Desktop\PP2_2025_SPRING\lecture0\racer\crash.wav').play()
        time.sleep(1)
        scr.fill("red")
        scr.blit(game_over, (30, 250))
        final_score = font_small.render("Your final score: " + str(SCORE), True, "black")
        scr.blit(final_score, (30, 320))
        pygame.display.update()
        time.sleep(4)
        pygame.quit()
        exit()
