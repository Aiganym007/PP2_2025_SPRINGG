...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        self.rect.center = (random.randint(40, W - 40), 0)  # Начальная позиция монеты сверху

    def move(self):
        global SCORE
        self.rect.move_ip(0, Speed)  # Двигаем монету вниз

        # Если монета выходит за нижнюю границу экрана, возвращаем её на верх
        if self.rect.bottom > 600:
            self.rect.top = 0
            self.rect.center = (random.randint(40, W - 40), 0)
...
while True:    
    ...
    if pygame.sprite.spritecollideany(P1, coins):
        ...
        SCORE += 1
        C1.rect.center = (random.randint(40, W - 40), 0)  # Перемещаем монету на верх экрана
